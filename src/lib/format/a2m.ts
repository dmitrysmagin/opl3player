...
export const ch_c = [
    ...
    new Uint16Array([
        ...
        0x103,0x10c,0x104,0x10d,0x105,0x013,0xff,0xff,0xff,0xff
    ])
];
...
export default class A2M extends FormatPlayer {
    ...
    private effSlot(slot: number, chan: number): number { return (slot * MAX_CHANNELS + chan); }
    private effSlot3(slot: number, chan: number): number { return (slot * MAX_CHANNELS + chan) * 3; }
    private effSlot4(slot: number, chan: number): number { return (slot * MAX_CHANNELS + chan) * 4; }
    private effSlot5(slot: number, chan: number): number { return (slot * MAX_CHANNELS + chan) * 5; }
    private effSlot2(slot: number, chan: number): number { return (slot * MAX_CHANNELS + chan) * 2; }
    ...
    private _macro_speedup(): number { return this.macro_speedup || 1; }
    ...
    private play_line_apply_global_fslide_row(event: number[], chan: number): void {
        for (let s = 0; s < 2; s++) {
            ...
            if (od === ef_Extended && ov === ef_ex_ExtendedCmd * 16 + ef_ex_cmd_ForceBpmSld) {
                if (def === ef_GlobalFSlideUp) this.update_playback_speed(val); else this.update_playback_speed(-val);
            } else {
                let eff: number;
                if (def === ef_GlobalFSlideUp) { eff = ef_FSlideUp; if (od === ef_Extended && ov === ef_ex_ExtendedCmd2 * 16 + ef_ex_cmd2_FTrm_XFGFS) eff = ef_GlobalFreqSlideUpXF; if (od === ef_Extended && ov === ef_ex_ExtendedCmd2 * 16 + ef_ex_cmd2_FVib_FGFS) eff = ef_FSlideUpFine; }
                else { eff = ef_FSlideDown; if (od === ef_Extended && ov === ef_ex_ExtendedCmd2 * 16 + ef_ex_cmd2_FTrm_XFGFS) eff = ef_GlobalFreqSlideDnXF; if (od === ef_Extended && ov === ef_ex_ExtendedCmd2 * 16 + ef_ex_cmd2_FVib_FGFS) eff = ef_FSlideDownFine; }
    ...
    private process_effects_prepare(event: number[], slot: number, chan: number): void {
        ...
        if (def !== ef_Vibrato && def !== ef_ExtraFineVibrato && def !== ef_TPortamVolSlide && def !== ef_VibratoVSlideFine) {
    ...
    private process_effects(event: number[], slot: number, chan: number): void {
        ...
        const ei = slot * 40 + chan * 2;
        switch (def) {
            case ef_Arpeggio: case ef_ExtraFineArpeggio: case ef_ArpggVSlide: case ef_ArpggVSlideFine: {
                if (def === ef_Arpeggio && !val) break;
                if (def === ef_Arpeggio) { this.chEffectTable[ei] = ef_Arpeggio; this.chEffectTable[ei + 1] = val; }
                else if (def === ef_ExtraFineArpeggio) { this.chEffectTable[ei] = ef_ExtraFineArpeggio; this.chEffectTable[ei + 1] = val; }
                ...
                const pa = pd === ef_Arpeggio || pd === ef_ExtraFineArpeggio || pd === ef_ArpggVSlide || pd === ef_ArpggVSlideFine;
                if (cn) {
            ...
            case ef_FSlideUp: case ef_FSlideDown: case ef_FSlideUpFine: case ef_FSlideDownFine:
                this.chEffectTable[ei] = def; this.chEffectTable[ei + 1] = val;
                this.chFslideTable[slot * 20 + chan] = val;
                break;
            case ef_FSlideUpVSlide: case ef_FSlUpFineVSlide: case ef_FSlideDownVSlide: case ef_FSlDownFineVSlide: case ef_FSlUpVSlF: case ef_FSlDownVSlF: case ef_FSlUpFineVSlF: case ef_FSlDownFineVSlF:
            ...
            case ef_TonePortamento: {
                ...
                if (hn || hc) {
                    if (event[3 + slot * 2]) { this.chEffectTable[ei] = ef_TonePortamento; this.chEffectTable[ei + 1] = val; }
                    else if (hc && this.chLastEffect[ei + 1]) { this.chEffectTable[ei] = ef_TonePortamento; this.chEffectTable[ei + 1] = this.chLastEffect[ei + 1]; }
                    else { this.chEffectTable[ei] = ef_TonePortamento; this.chEffectTable[ei + 1] = 0; }
            ...
            case ef_VibratoVolSlide: case ef_TPortamVSlideFine: this.update_effect_table(slot, chan, 7, def, val); break;
            case ef_Vibrato: case ef_ExtraFineVibrato: {
                this.update_effect_table(slot, chan, 4, def, val);
                if (event[4 - slot * 2] === ef_Extended && event[5 - slot * 2] === ef_ex_ExtendedCmd2 * 16 + ef_ex_cmd2_FVib_FGFS) this.chVibrTable[slot * 100 + chan * 5 + 4] = 1;
                const m = this.chEffectTable[ei + 1];
                this.chVibrTable[slot * 100 + chan * 5 + 1] = (m / 16) & 0xff;
                this.chVibrTable[slot * 100 + chan * 5 + 2] = m % 16;
                break;
            }
            case ef_Tremolo: case ef_ExtraFineTremolo: {
                this.update_effect_table(slot, chan, 5, def, val);
                if (event[4 - slot * 2] === ef_Extended && event[5 - slot * 2] === ef_ex_ExtendedCmd2 * 16 + ef_ex_cmd2_FTrm_XFGFS) this.chTremTable[slot * 100 + chan * 5 + 4] = 1;
                const m = this.chEffectTable[ei + 1];
                this.chTremTable[slot * 100 + chan * 5 + 1] = (m / 16) & 0xff;
                this.chTremTable[slot * 100 + chan * 5 + 2] = m % 16;
                break;
            }
            case ef_TPortamVolSlide: case ef_VibratoVSlideFine: {
                this.update_effect_table(slot, chan, 6, def, val);
                if (event[4 - slot * 2] === ef_Extended && event[5 - slot * 2] === ef_ex_ExtendedCmd2 * 16 + ef_ex_cmd2_FVib_FGFS) this.chVibrTable[slot * 100 + chan * 5 + 4] = 1;
                break;
            }
            case ef_SetCarrierVol: this.set_ins_volume(0xff, 63 - val, chan); this.chEffectTable[ei] = 0; break;
            case ef_SetModulatorVol: this.set_ins_volume(63 - val, 0xff, chan); this.chEffectTable[ei] = 0; break;
            ...
            case ef_PatternBreak:
                if (this.no_loop(chan, this.current_line)) { this.pattern_break = true; this.pattern_break_from_bxx = false; this.next_line = Math.min(val, this.songinfo.patt_len - 1); }
                this.chEffectTable[ei] = 0; break;
            case ef_SetSpeed: this.speed = val; this.chEffectTable[ei] = 0; break;
            case ef_SetTempo: this.update_timer(val); this.chEffectTable[ei] = 0; break;
            case ef_SetWaveform:
                if ((val >> 4) <= 7) { this.chFmparTable[chan].wformC = (val >> 4) & 0xf; this.update_carrier_adsrw(chan); }
                if ((val & 0xf) <= 7) { this.chFmparTable[chan].wformM = val & 0xf; this.update_modulator_adsrw(chan); }
                this.chEffectTable[ei] = 0; break;
            case ef_VolSlide: case ef_VolSlideFine: this.chEffectTable[ei] = def; this.chEffectTable[ei + 1] = val; break;
            case ef_RetrigNote:
                if (val) {
            ...
            case ef_SetGlobalVolume: this.global_volume = val; this.set_global_volume(); this.chEffectTable[ei] = 0; break;
            case ef_Tremor:
                if (val) {
            ...
            case ef_Extended:
                switch (val >> 4) {
                    case ef_ex_SetTremDepth:
                        if (val & 0xf) { this.opl3out(0xbd, this.misc_register | 0x80); this.current_tremolo_depth = 1; }
                        else { this.opl3out(0xbd, this.misc_register & 0x7f); this.current_tremolo_depth = 0; } break;
                    case ef_ex_SetVibDepth:
                        if (val & 0xf) { this.opl3out(0xbd, this.misc_register | 0x40); this.current_vibrato_depth = 1; }
                        else { this.opl3out(0xbd, this.misc_register & 0xbf); this.current_vibrato_depth = 0; } break;
                    case ef_ex_SetAttckRateM: this.chFmparTable[chan].attckM = val & 0xf; this.update_modulator_adsrw(chan); break;
                    case ef_ex_SetDecayRateM: this.chFmparTable[chan].decM = val & 0xf; this.update_modulator_adsrw(chan); break;
                    case ef_ex_SetSustnLevelM: this.chFmparTable[chan].sustnM = val & 0xf; this.update_modulator_adsrw(chan); break;
                    case ef_ex_SetRelRateM: this.chFmparTable[chan].relM = val & 0xf; this.update_modulator_adsrw(chan); break;
                    case ef_ex_SetAttckRateC: this.chFmparTable[chan].attckC = val & 0xf; this.update_carrier_adsrw(chan); break;
                    case ef_ex_SetDecayRateC: this.chFmparTable[chan].decC = val & 0xf; this.update_carrier_adsrw(chan); break;
                    case ef_ex_SetSustnLevelC: this.chFmparTable[chan].sustnC = val & 0xf; this.update_carrier_adsrw(chan); break;
                    case ef_ex_SetRelRateC: this.chFmparTable[chan].relC = val & 0xf; this.update_carrier_adsrw(chan); break;
                    case ef_ex_SetFeedback: this.chFmparTable[chan].feedb = val & 0xf; this.update_fmpar(chan); break;
                    case ef_ex_SetPanningPos: this.chPanningTable[chan] = val & 0xf; this.update_fmpar(chan); break;
                    case ef_ex_PatternLoop: case ef_ex_PatternLoopRec:
                        if (!(val & 0xf)) this.chLoopbckTable[chan] = this.current_line;
                        else if (this.chLoopbckTable[chan] !== 0xff) {
                            if (this.chLoopTable[chan * 256 + this.current_line] === 0xff) this.chLoopTable[chan * 256 + this.current_line] = val & 0xf;
                            ...
                            } else if (val >> 4 === ef_ex_PatternLoopRec) this.chLoopTable[chan * 256 + this.current_line] = 0xff;
                        }
                        break;
                    case ef_ex_ExtendedCmd:
                        switch (val & 0xf) {
                            case ef_ex_cmd_MKOffLoopDi: this.chKeyoffLoop[chan] = 0; break;
                            case ef_ex_cmd_MKOffLoopEn: this.chKeyoffLoop[chan] = 1; break;
                            case ef_ex_cmd_TPortaFKdis: this.chPortaFK[chan] = 0; break;
                            case ef_ex_cmd_TPortaFKenb: this.chPortaFK[chan] = 1; break;
                            case ef_ex_cmd_RestartEnv: this.key_on(chan); this.change_freq(chan, this.chFreqTable[chan]); break;
                            case ef_ex_cmd_4opVlockOff: if (this.is_4op_chan(chan)) { this.chVol4opLock[chan] = 0; const i = this.is_4op_chan_hi(chan) ? 1 : -1; this.chVol4opLock[chan + i] = 0; } break;
                            case ef_ex_cmd_4opVlockOn: if (this.is_4op_chan(chan)) { this.chVol4opLock[chan] = 1; const i = this.is_4op_chan_hi(chan) ? 1 : -1; this.chVol4opLock[chan + i] = 1; } break;
                        } break;
                    case ef_ex_ExtendedCmd2:
                        switch (val & 0xf) {
                            case ef_ex_cmd2_RSS: this.release_sustaining_sound(chan); break;
                            case ef_ex_cmd2_ResetVol: this.reset_ins_volume(chan); break;
                            case ef_ex_cmd2_LockVol: this.chVolumeLock[chan] = 1; break;
                            case ef_ex_cmd2_UnlockVol: this.chVolumeLock[chan] = 0; break;
                            case ef_ex_cmd2_LockVP: this.chPeakLock[chan] = 1; break;
                            case ef_ex_cmd2_UnlockVP: this.chPeakLock[chan] = 0; break;
                            case ef_ex_cmd2_VSlide_mod: this.chVolslideType[chan] = 0; break;
                            case ef_ex_cmd2_LockPan: this.chPanLock[chan] = 1; break;
                            case ef_ex_cmd2_UnlockPan: this.chPanLock[chan] = 0; break;
                            case ef_ex_cmd2_VibrOff: this.change_frequency(chan, this.chFreqTable[chan]); break;
                            case ef_ex_cmd2_TremOff:
                                ...
                                break;
                            case ef_ex_cmd2_VSlide_car: this.chVolslideType[chan] = (event[4 - slot * 2] === ef_Extended && event[5 - slot * 2] === ef_ex_ExtendedCmd2 * 16 + ef_ex_cmd2_VSlide_def) ? 3 : 1; break;
                            case ef_ex_cmd2_VSlide_def: this.chVolslideType[chan] = (event[4 - slot * 2] === ef_Extended && event[5 - slot * 2] === ef_ex_ExtendedCmd2 * 16 + ef_ex_cmd2_VSlide_car) ? 3 : 2; break;
                        } break;
                } break;
            case ef_Extended2:
                switch (val >> 4) {
                    case ef_ex2_PatDelayFrame: this.pattern_delay = true; this.tickD = val & 0xf; break;
                    case ef_ex2_PatDelayRow: this.pattern_delay = true; this.tickD = this.speed * (val & 0xf); break;
                    case ef_ex2_NoteDelay: this.chEffectTable[ei] = ef_Extended2; this.chEffectTable[ei + 1] = val; this.chNotedelTable[chan] = val & 0xf; break;
                    case ef_ex2_NoteCut: this.chEffectTable[ei] = ef_Extended2; this.chEffectTable[ei + 1] = val; this.chNotecutTable[chan] = val & 0xf; break;
                    case ef_ex2_FineTuneUp: this.chFtuneTable[chan] += val & 0xf; break;
                    case ef_ex2_FineTuneDown: this.chFtuneTable[chan] -= val & 0xf; break;
                    default: this.chEffectTable[ei] = ef_Extended2; this.chEffectTable[ei + 1] = val; break;
                } break;
            case ef_Extended3:
                switch (val >> 4) {
                    case ef_ex3_SetConnection: this.chFmparTable[chan].connect = val & 0xf; this.update_fmpar(chan); break;
                    case ef_ex3_SetMultipM: this.chFmparTable[chan].multipM = val & 0xf; this.update_fmpar(chan); break;
                    case ef_ex3_SetKslM: this.chFmparTable[chan].kslM = val & 0xf; this.update_fmpar(chan); break;
                    case ef_ex3_SetTremoloM: this.chFmparTable[chan].tremM = val & 0xf; this.update_fmpar(chan); break;
                    case ef_ex3_SetVibratoM: this.chFmparTable[chan].vibrM = val & 0xf; this.update_fmpar(chan); break;
                    case ef_ex3_SetKsrM: this.chFmparTable[chan].ksrM = val & 0xf; this.update_fmpar(chan); break;
                    case ef_ex3_SetSustainM: this.chFmparTable[chan].sustM = val & 0xf; this.update_fmpar(chan); break;
                    case ef_ex3_SetMultipC: this.chFmparTable[chan].multipC = val & 0xf; this.update_fmpar(chan); break;
                    case ef_ex3_SetKslC: this.chFmparTable[chan].kslC = val & 0xf; this.update_fmpar(chan); break;
                    case ef_ex3_SetTremoloC: this.chFmparTable[chan].tremC = val & 0xf; this.update_fmpar(chan); break;
                    case ef_ex3_SetVibratoC: this.chFmparTable[chan].vibrC = val & 0xf; this.update_fmpar(chan); break;
                    case ef_ex3_SetKsrC: this.chFmparTable[chan].ksrC = val & 0xf; this.update_fmpar(chan); break;
                    case ef_ex3_SetSustainC: this.chFmparTable[chan].sustC = val & 0xf; this.update_fmpar(chan); break;
                } break;
        }
    }

    private new_process_note(event: number[], chan: number): void {
        const tp = (event[2] === ef_TonePortamento || event[2] === ef_VibratoVolSlide || event[2] === ef_TPortamVSlideFine || event[4] === ef_TonePortamento || event[4] === ef_VibratoVolSlide || event[4] === ef_TPortamVSlideFine);
        const nd = (event[2] === ef_Extended2 && (event[3] / 16 === ef_ex2_NoteDelay)) || (event[4] === ef_Extended2 && (event[5] / 16 === ef_ex2_NoteDelay));
        const dn = tp || nd;
        ...
        if (!dn) { this.output_note(event[0], this.chVoiceTable[chan], chan, true, no_swap_and_restart(event)); return; }
    ...
    private output_note(note: number, ins: number, chan: number, rm: boolean, ra: boolean): void {
        ...
        if (note) {
            ...
            if (rm) {
                const fnr = (this.chEventTable[chan * 6 + 2] === ef_Extended && this.chEventTable[chan * 6 + 3] === ef_ex_ExtendedCmd2 * 16 + ef_ex_cmd2_NoRestart) ||
                           (this.chEventTable[chan * 6 + 4] === ef_Extended && this.chEventTable[chan * 6 + 5] === ef_ex_ExtendedCmd2 * 16 + ef_ex_cmd2_NoRestart);
    ...
    private get_4op_data(chan: number): { mode: boolean; conn: number; ch1: number; ch2: number; ins1: number; ins2: number } {
    ...
    private update_effects_slot(slot: number, chan: number): void {
        ...
        switch (def) {
            case ef_Arpeggio: if (val) this.arpeggio(slot, chan); break;
            case ef_ArpggVSlide: this.volume_slide(chan, (val >> 4) & 0xf, val & 0xf); this.arpeggio(slot, chan); break;
            case ef_ArpggVSlideFine: this.arpeggio(slot, chan); break;
            case ef_FSlideUp: this.portamento_up(chan, val, nFreq(97)); break;
            case ef_FSlideDown: this.portamento_down(chan, val, nFreq(0)); break;
            case ef_FSlideUpVSlide: this.portamento_up(chan, this.chFslideTable[slot * 20 + chan], nFreq(97)); this.volume_slide(chan, (val >> 4) & 0xf, val & 0xf); break;
            case ef_FSlUpFineVSlide: this.volume_slide(chan, (val >> 4) & 0xf, val & 0xf); break;
            case ef_FSlideDownVSlide: this.portamento_down(chan, this.chFslideTable[slot * 20 + chan], nFreq(0)); this.volume_slide(chan, (val >> 4) & 0xf, val & 0xf); break;
            case ef_FSlDownFineVSlide: this.volume_slide(chan, (val >> 4) & 0xf, val & 0xf); break;
            case ef_FSlUpVSlF: this.portamento_up(chan, this.chFslideTable[slot * 20 + chan], nFreq(97)); break;
            case ef_FSlDownVSlF: this.portamento_down(chan, this.chFslideTable[slot * 20 + chan], nFreq(0)); break;
            case ef_TonePortamento: this.tone_portamento(slot, chan); break;
            case ef_VibratoVolSlide: this.volume_slide(chan, (val >> 4) & 0xf, val & 0xf); this.tone_portamento(slot, chan); break;
            case ef_TPortamVSlideFine: this.tone_portamento(slot, chan); break;
            case ef_Vibrato: if (!this.chVibrTable[slot * 100 + chan * 5 + 4]) this.vibrato(slot, chan); break;
            case ef_Tremolo: if (!this.chTremTable[slot * 100 + chan * 5 + 4]) this.tremolo(slot, chan); break;
            case ef_TPortamVolSlide: this.volume_slide(chan, (val >> 4) & 0xf, val & 0xf); if (!this.chVibrTable[slot * 100 + chan * 5 + 4]) this.vibrato(slot, chan); break;
            case ef_VibratoVSlideFine: if (!this.chVibrTable[slot * 100 + chan * 5 + 4]) this.vibrato(slot, chan); break;
            case ef_VolSlide: this.volume_slide(chan, (val >> 4) & 0xf, val & 0xf); break;
            case ef_RetrigNote:
            ...
            case ef_Extended2:
                switch (val >> 4) {
                    ...
                    case ef_ex2_NoteCut:
                        ...
                        break;
                    case ef_ex2_GlVolSlideUp: this.global_volume_slide(val & 0xf, 0xff); break;
                    case ef_ex2_GlVolSlideDn: this.global_volume_slide(0xff, val & 0xf); break;
                }
                break;
        }
    }
    ...
    private update_fine_effects(slot: number, chan: number): void {
        const ei = slot * 40 + chan * 2, def = this.chEffectTable[ei], val = this.chEffectTable[ei + 1];
        switch (def) {
            case ef_ArpggVSlideFine: this.volume_slide(chan, (val >> 4) & 0xf, val & 0xf); break;
            case ef_FSlideUpFine: this.portamento_up(chan, val, nFreq(97)); break;
            case ef_FSlideDownFine: this.portamento_down(chan, val, nFreq(0)); break;
            case ef_FSlUpVSlF: case ef_FSlDownVSlF: this.volume_slide(chan, (val >> 4) & 0xf, val & 0xf); break;
            case ef_FSlUpFineVSlide: this.portamento_up(chan, this.chFslideTable[slot * 20 + chan], nFreq(97)); break;
            case ef_FSlDownFineVSlide: this.portamento_down(chan, this.chFslideTable[slot * 20 + chan], nFreq(0)); break;
            case ef_FSlUpFineVSlF: this.portamento_up(chan, this.chFslideTable[slot * 20 + chan], nFreq(97)); this.volume_slide(chan, (val >> 4) & 0xf, val & 0xf); break;
            case ef_FSlDownFineVSlF: this.portamento_down(chan, this.chFslideTable[slot * 20 + chan], nFreq(0)); this.volume_slide(chan, (val >> 4) & 0xf, val & 0xf); break;
            case ef_TPortamVSlideFine: this.volume_slide(chan, (val >> 4) & 0xf, val & 0xf); break;
            case ef_Vibrato: if (this.chVibrTable[slot * 100 + chan * 5 + 4]) this.vibrato(slot, chan); break;
            case ef_Tremolo: if (this.chTremTable[slot * 100 + chan * 5 + 4]) this.tremolo(slot, chan); break;
            case ef_TPortamVolSlide: if (this.chVibrTable[slot * 100 + chan * 5 + 4]) this.vibrato(slot, chan); break;
            case ef_VibratoVSlideFine: this.volume_slide(chan, (val >> 4) & 0xf, val & 0xf); if (this.chVibrTable[slot * 100 + chan * 5 + 4]) this.vibrato(slot, chan); break;
            case ef_VolSlideFine: this.volume_slide(chan, (val >> 4) & 0xf, val & 0xf); break;
            case ef_Extended2:
                switch (val >> 4) { case ef_ex2_GlVolSlideUpF: this.global_volume_slide(val & 0xf, 0xff); break; case ef_ex2_GlVolSlideDnF: this.global_volume_slide(0xff, val & 0xf); break; }
                break;
        }
    }
    ...
    private update_extra_fine_effects_slot(slot: number, chan: number): void {
        ...
        switch (def) {
            case ef_Extended2:
                switch (val >> 4) {
                    case ef_ex2_GlVolSldUpXF: this.global_volume_slide(val & 0xf, 0xff); break;
                    case ef_ex2_GlVolSldDnXF: this.global_volume_slide(0xff, val & 0xf); break;
                    case ef_ex2_VolSlideUpXF: this.volume_slide(chan, val & 0xf, 0); break;
                    case ef_ex2_VolSlideDnXF: this.volume_slide(chan, 0, val & 0xf); break;
                    case ef_ex2_FreqSlideUpXF: this.portamento_up(chan, val & 0xf, nFreq(97)); break;
                    case ef_ex2_FreqSlideDnXF: this.portamento_down(chan, val & 0xf, nFreq(0)); break;
                } break;
            case ef_GlobalFreqSlideUpXF: this.portamento_up(chan, val, nFreq(97)); break;
            case ef_GlobalFreqSlideDnXF: this.portamento_down(chan, val, nFreq(0)); break;
            case ef_ExtraFineArpeggio: this.arpeggio(slot, chan); break;
            case ef_ExtraFineVibrato: if (!this.chVibrTable[slot * 100 + chan * 5 + 4]) this.vibrato(slot, chan); break;
            case ef_ExtraFineTremolo: if (!this.chTremTable[slot * 100 + chan * 5 + 4]) this.tremolo(slot, chan); break;
        }
    }
    ...
    private check_swap_arp_vibr(event: number[], slot: number, chan: number): void {
        const nr = (event[4 - slot * 2] === ef_Extended && event[5 - slot * 2] === ef_ex_ExtendedCmd2 * 16 + ef_ex_cmd2_NoRestart);
        const mo = chan * MACRO_TABLE_STRIDE;
        switch (event[2 + slot * 2]) {
            ...
            case ef_SwapVibrato:
                ...
                break;
            case ef_SetCustomSpeedTab: this.generate_custom_vibrato(event[3 + slot * 2]); break;
        }
    }
    ...
    private is_data_empty2(instr: any): boolean {
        return !instr.fm.attckM && !instr.fm.attckC && !instr.fm.decM && !instr.fm.decC &&
               !instr.fm.sustnM && !instr.fm.sustnC && !instr.fm.relM && !instr.fm.relC;
    }
...
function no_swap_and_restart(event: number[]): boolean {
    ...
    return !(((event[4] === ef_SwapArpeggio || event[4] === ef_SwapVibrato) &&
               event[2] === ef_Extended && event[3] === NO_RESTART)) &&
           !(((event[2] === ef_SwapArpeggio || event[2] === ef_SwapVibrato) &&
               event[4] === ef_Extended && event[5] === NO_RESTART));
}
